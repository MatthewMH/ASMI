...
def print_mikroskopik_view(request, item_id):
    item = get_object_or_404(GambarMikroskopikSputum, id=item_id)

    patient_to_print = GambarMikroskopikSputum.objects.filter(id=item_id)

    context = {'patient_to_print': patient_to_print}

    return render(request, "inference_print_mikroskopik.html", context)
...
def delete_mikroskopik_view(request, item_id):
    item = get_object_or_404(GambarMikroskopikSputum, id=item_id)

    patient_to_delete = GambarMikroskopikSputum.objects.filter(id=item_id)
    ...
    if action == 'verify':
    	selected_patient = request.POST.get('idPrint')
    	patient_to_delete = GambarMikroskopikSputum.objects.filter(id=selected_patient)

    	entered_password = request.POST.get('password')
    	user = request.user
    	if user.check_password(entered_password):
    		patient = get_object_or_404(GambarMikroskopikSputum, id=selected_patient)
    		patient.delete()
    		messages.success(request, 'Data Berhasil dihapus')
    		return redirect('../admin/hibahs/gambarmikroskopiksputum/')
    	else:
    		messages.error(request, 'Password salah')
    		return render(request, "delete_image.html", {'password_error': True, 'patient_to_delete': patient_to_delete})
